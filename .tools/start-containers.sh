#!/bin/bash
# Purpose: Start Mutillidae Docker containers defined in docker-compose.yml
# Usage: ./start-containers.sh [options]
# Description: This script is used to start and optionally initialize the Mutillidae containers.
...
show_help() {
    ...
    echo ""
    echo "Description:"
    echo "This script is used to start and optionally initialize the Mutillidae containers."
    ...
    echo "  3. Request the Mutillidae database to be built."
    echo "  4. Upload the specified Mutillidae LDIF file to the LDAP directory server."
    echo ""
...
if [ "$INITIALIZE_CONTAINERS" = true ]; then
    ...
    # Request Mutillidae database to be built
    print_message "Requesting Mutillidae database be built"
    curl -sS http://mutillidae.localhost/set-up-database.php || handle_error "Failed to set up the Mutillidae database"
